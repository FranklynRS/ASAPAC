...
export const AcertosService = {
	...
	async fetchLancamentosByMes(idMes: number): Promise<Lancamento[]> {
		try {
			const token = AuthService.getToken();
			if (!token) throw new Error('Token de autenticação não encontrado.');
			const response = await fetch(`${API_BASE_URL}/lancamentos/mes/${idMes}`, {
				method: 'GET',
				headers: {
	...
	async fetchAcertosByMes(idMes: number): Promise<Acerto[]> {
		try {
			const token = AuthService.getToken();
			if (!token) throw new Error('Token de autenticação não encontrado.');
			const response = await fetch(`${API_BASE_URL}/acertos/mes/${idMes}`, {
				method: 'GET',
				headers: {
					'Content-Type': 'application/json',
					'Authorization': `Bearer ${token}`,
	...
	async fetchCombinedByMes(idMes: number): Promise<Lancamento[]> {
		try {
			const token = AuthService.getToken();
			if (!token) throw new Error('Token de autenticação não encontrado.');
			const response = await fetch(`${API_BASE_URL}/lancamentos-combinados/${idMes}`, {
				method: 'GET',
				headers: {
					'Content-Type': 'application/json',
					'Authorization': `Bearer ${token}`,
	...
	async createAcerto(acertosData: AcertoData): Promise<any> {
		try {
			const token = AuthService.getToken();
			if (!token) throw new Error('Token de autenticação não encontrado.');
			const response = await fetch('${API_BASE_URL}/acertos', {
				method: 'POST',
				headers: {
					'Content-Type': 'application/json',
					'Authorization': `Bearer ${token}`,
	...
	async updateAcerto(id: number, acertoData: Partial<AcertoData>): Promise<any> {
		try {
			const token = AuthService.getToken();
			if (!token) throw new Error('Token de autenticação não encontrado.');
			const response = await fetch(`${API_BASE_URL}/acertos/${id}`, {
				method: 'PUT',
				headers: {
					'Content-Type': 'application/json',
	...
	async updateLancamento(id: number | string, lancamentoData: Partial<LancamentoData>): Promise<any> {
		try {
			...
			const response = await fetch(`${API_BASE_URL}/lancamentos/${realId}`, {
				method: 'PUT',
				headers: {
					'Content-Type': 'application/json',
					'Accept': 'application/json',
			...
			if (!response.ok) {
				...
				console.error('Erro retornado pelo backend:', erroBackend);
	...
	async deleteLancamento(id: number | string): Promise<void> {
		try {
			...
			const realId = String(id).replace('lancamento_', '');
